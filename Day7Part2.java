...
public class Day7Part2 {
    ...
    static Hand classifyHand(String handString) {
        ...
        var numPairs = values.stream().filter(value -> value == 2).count();
        var hasTwoPairs = numPairs == 2;
        var hasHouse = (numJokers == 0 && values.contains(3) && values.contains(2))
                || (numJokers == 1 && hasTwoPairs);

        if (numJokers == 5 || values.contains(5 - numJokers)) {
            return new Hand(HandType.FIVE_EQ, handString);
        } else if (values.contains(4 - numJokers)) {
            return new Hand(HandType.FOUR_EQ, handString);
        } else if (hasHouse) {
            return new Hand(HandType.HOUSE, handString);
        } else if (values.contains(3 - numJokers)) {
            return new Hand(HandType.THREE_EQ, handString);
        } else if (hasTwoPairs) {
            return new Hand(HandType.TWO_PAIR, handString);
        } else if (values.contains(2 - numJokers)) {
    ...
    static {
        cardValueMap.putAll(Map.of("A", 14, "K", 13, "Q", 12, "T", 10, "J", 1));
        for (int i = 2; i < 10; i++) {
